...
import json
import uuid
from PyQt5.QtWidgets import QApplication, QWidget, QVBoxLayout, QLabel, QPushButton, QFileDialog, QComboBox, QMessageBox, QTableWidget, QTableWidgetItem, QHeaderView, QCheckBox, QLineEdit, QDialog, QInputDialog, QTableView
...
class ConvertisseurCSV(QWidget):
    ...
    def initUI(self):
        ...
        # Ajout du bouton "Historique des versions"
        self.btn_historique_versions = QPushButton("Afficher historique des versions", self)
        self.btn_historique_versions.clicked.connect(self.afficher_historique_versions)
        layout.addWidget(self.btn_historique_versions)
        ...
        for row in range(self.table_widget.rowCount()):
            ...
            for col in range(1, self.table_widget.columnCount()):
                ...
                if item and texte_recherche in item.text().lower():
                    hide_row = False
                    item.setBackground(Qt.yellow)  # Mise en évidence des résultats de recherche
                    break
                else:
    ...
    def afficher_historique_versions(self):
        historique_dialog = QDialog(self)
        historique_dialog.setWindowTitle("Historique des versions")
        historique_dialog.setGeometry(100, 100, 400, 300)
        layout = QVBoxLayout()

        versions = os.listdir('.')
        versions_csv = [fichier for fichier in versions if fichier.startswith('backup_version_') and fichier.endswith('.csv')]

        if versions_csv:
            table_versions = QTableWidget(len(versions_csv), 1, historique_dialog)
            table_versions.setHorizontalHeaderLabels(['Version sauvegardée'])
            for i, version in enumerate(versions_csv):
                table_versions.setItem(i, 0, QTableWidgetItem(version))
            table_versions.horizontalHeader().setSectionResizeMode(QHeaderView.Stretch)
            layout.addWidget(table_versions)
        else:
            layout.addWidget(QLabel("Aucune version sauvegardée trouvée."))

        historique_dialog.setLayout(layout)
        historique_dialog.exec_()
